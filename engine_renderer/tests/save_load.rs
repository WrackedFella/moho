...
fn save_and_load_scene_roundtrip() {
    ...
    let s1 = Sphere::new(Vec3::new(0.0, 0.0, 0.0), 1.0, MaterialType::Lambertian { albedo: Vec3::new(0.1, 0.2, 0.3) });
    let s2 = Sphere::new(Vec3::new(2.0, 0.0, 0.0), 0.5, MaterialType::Metal { albedo: Vec3::new(0.7, 0.7, 0.7), fuzz: 0.2 });
    world.push((s1,));
    world.push((s2,));
    ...
    scene2.load_from_file(&path, &mut loaded_world).expect("load ok");
    ...
    assert_eq!(orig_count, loaded_count, "scene roundtrip should preserve entity count");
