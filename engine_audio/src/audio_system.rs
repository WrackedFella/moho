...
use crate::audio_source::{AudioSource, AudioCategory};
use crate::audio_events::{AudioEvent, AudioCategory as EventCategory};
...
impl AudioSystem {
    ...
    fn preload_ui_sounds(&mut self) -> AudioResult<()> {
        ...
        for sound_path in ui_sounds {
            if let Ok(audio_data) = std::fs::read(sound_path) {
                self.ui_sound_cache.insert(sound_path.to_string(), audio_data);
    ...
    pub fn play_audio_source(&mut self, source: &AudioSource) -> AudioResult<()> {
        ...
        if effective_volume <= 0.0 {
            debug!("Skipping audio playback due to zero volume: {}", source.path);
            return Ok(());
        }
        ...
        let source_decoder = rodio::Decoder::new(cursor)
            .map_err(|e| AudioError::LoadFailed(format!("Failed to decode {}: {}", source.path, e)))?;
        ...
        debug!("Playing audio: {} at volume {:.2}", source.path, effective_volume);
        Ok(())
    }
    ...
    pub fn handle_event(&mut self, event: AudioEvent) -> AudioResult<()> {
        match event {
            ...
            AudioEvent::MenuNavigate => {
                self.play_ui_sound("sfx/button_click.mp3", 0.6)
            },
            AudioEvent::Confirm => {
                self.play_ui_sound("sfx/button_click.mp3", 0.8)
            },
            AudioEvent::Cancel => {
                self.play_ui_sound("sfx/button_click.mp3", 0.7)
            },
            AudioEvent::Error => {
                self.play_ui_sound("sfx/button_click.mp3", 0.9)
            },
            AudioEvent::CustomSound { path, volume } => {
                self.play_sound(&path, volume)
            },
            AudioEvent::BackgroundMusic { path, volume, looped } => {
                let source = AudioSource::background_music(&path, volume)
                    .with_looping(looped);
                self.play_audio_source(&source)
            },
    ...
    fn play_ui_sound<P: AsRef<Path>>(&mut self, path: P, volume: f32) -> AudioResult<()> {
        ...
        let effective_volume = self.settings.effective_volume(&AudioCategory::UserInterface) * volume;
        ...
        debug!("Playing UI audio: {} at volume {:.2}", path_str, effective_volume);
        Ok(())
    }
    ...
    fn get_ui_sound_data(&mut self, path: &str) -> AudioResult<Vec<u8>> {
        ...
        self.ui_sound_cache.insert(path.to_string(), audio_data.clone());
    ...
    fn load_audio_file(&mut self, path: &str) -> AudioResult<Vec<u8>> {
        ...
        self.audio_cache.insert(path.to_string(), audio_data.clone());
