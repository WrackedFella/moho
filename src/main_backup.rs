        ...
        let _ = event_loop.run(move |event, active_event_loop| {
            match event {
                                ...
                                while let Ok(ev) = ui_receiver.try_recv() {
                                    match ev {
                                        ...
                                        UiEvent::ShowMenu(name) => {
                                            ...
                                            // Placeholder: do nothing except log for now.
                                        }
                ...
                Event::WindowEvent { event, .. } => {
                    // Forward only mouse and modifier events to the UI adapter (no keyboard)
                    #[cfg(feature = "ui-egui")]
                    {
                        use winit::event::WindowEvent as WEvent;
                        
                        // Check if overlay is visible before forwarding events
                        let overlay_visible = if let Ok(adapter) = ui_adapter.lock() {
                            adapter.is_visible()
                        } else {
                            false
                        };
                        
                        // Only forward mouse events when the UI overlay is visible
                        // Ignore all keyboard input for now
                        if overlay_visible {
                            match &event {
                                WEvent::CursorMoved { .. }
                                | WEvent::MouseInput { .. }
                ...
                Event::DeviceEvent {
                    ...
                    if !cursor_is_grabbed || overlay_visible {
                        ...
                        log::debug!("Ignoring mouse motion: cursor_grabbed={}, overlay_visible={}", cursor_is_grabbed, overlay_visible);
                    } else {
                        ...
                        // DISABLED: This might interfere with menu handling
                        // simulate(&mut world, sim_dt);
