...
impl Binding {
    pub fn new(code: u32, mods: u8) -> Self { Self { code, mods } }
}

impl Default for Binding {
    fn default() -> Self { Binding::new('W' as u32, 0) }
}
...
impl Prefs {
    pub fn config_path() -> PathBuf { PathBuf::from("config/prefs.ini") }
        ...
        if let Ok(map) = ini::macro_safe_read(&content) {
            if let Some(section) = map.get("prefs").or_else(|| map.get("default")) {
                ...
                let get_f32 = |k: &str, def: f32| {
                    section.get(k).and_then(|o| o.clone()).and_then(|s| s.parse::<f32>().ok()).unwrap_or(def)
                };
                ...
                fn parse_binding(s: &str, fallback: Binding) -> Binding {
                    ...
                    if s.eq_ignore_ascii_case("unbound") { return Binding::new(0, 0); }
                    let mut mods: u8 = 0;
                    ...
                    for p in &parts {
                        ...
                        match up.as_str() {
                            "CTRL" | "CONTROL" => { mods |= 1; continue; }
                            "SHIFT" => { mods |= 2; continue; }
                            "ALT" => { mods |= 4; continue; }
                            _ => { key_part = p.trim(); }
                        }
                    }
                    if key_part.is_empty() { return fallback; }
                ...
                if let Some(s) = get_str("key_w") { prefs.key_w = parse_binding(&s, prefs.key_w); }
                if let Some(s) = get_str("key_a") { prefs.key_a = parse_binding(&s, prefs.key_a); }
