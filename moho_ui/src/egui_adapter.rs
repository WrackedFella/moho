        ...
        fn event_is_escape(ev: &winit::event::WindowEvent) -> bool {
            ...
            match ev {
                WindowEvent::KeyboardInput { event, .. } => {
                    ...
                    if event.text.as_deref().map(|s| s == "\u{1b}").unwrap_or(false) {
                        return true;
                    }
        ...
        pub fn new(window: Option<Arc<Window>>) -> (Self, UiReceiver) {
            ...
            // can interact with the menu immediately.
                ...
                if ui.ui_visible {
                    ...
                    ui.raw_input.events.push(egui::Event::PointerMoved(egui::pos2(tx, ty)));
    ...
    impl FrameCallback for EguiUi {
        fn call(
            ...
            if let (Some(win), Some(grabbed)) = (self.window.as_ref(), self.cursor_grabbed) {
                ...
                let _ = win.set_cursor_grab(if grabbed { CursorGrabMode::Locked } else { CursorGrabMode::None });
            }
            ...
            if self.ui_visible {
                ...
                if self.warmup_pending {
                    if cfg!(feature = "ui-egui-debug") {
                        log::info!("EguiUi: performing warmup clear/pass (start_mode={})", self.start_mode);
                    }
                    ...
                    if let Some(ctx) = &self.egui_ctx {
                        ...
                        if self.egui_renderer.is_none() {
                            if let Some(fmt) = self.surface_format {
                                if cfg!(feature = "ui-egui-debug") {
                                    log::info!("EguiUi: creating egui_wgpu::Renderer for warmup with format {:?}", fmt);
                                }
                        ...
                        let full_output = ctx.run(input, |ctx| {
                            egui::CentralPanel::default().show(ctx, |ui| {
                                ...
                                if cfg!(feature = "ui-egui-debug") {
                                    let painter = ctx.layer_painter(egui::LayerId::new(egui::Order::Foreground, egui::Id::new("moho_debug_rects_warmup")));
                                    let load_rect = egui::Rect::from_min_max(egui::pos2(8.0, 8.0), egui::pos2(128.0, 40.0));
                                    let exit_rect = egui::Rect::from_min_max(egui::pos2(8.0, 48.0), egui::pos2(128.0, 80.0));
                                    painter.rect_filled(load_rect, 0.0, egui::Color32::from_rgba_unmultiplied(255, 0, 0, 60));
                                    // outline omitted (egui API mismatch); filled rect is sufficient for debug
                                    painter.rect_filled(exit_rect, 0.0, egui::Color32::from_rgba_unmultiplied(0, 255, 0, 60));
                                    // outline omitted (egui API mismatch); filled rect is sufficient for debug
                                }
                            });
                        });
                ...
                if let Some(ctx) = &self.egui_ctx {
                    ...
                    let full_output = ctx.run(input, |ctx| {
                        ...
                        if cfg!(feature = "ui-egui-debug") {
                            // Inspect the available area for the top-left anchored Area
                            // This helps diagnose why an anchored Area might produce no shapes
                            log::info!("EguiUi: ctx.available_rect = {:?}", ctx.available_rect());
                        }
                        if USE_AREA {
                            egui::Area::new("moho_area".into())
                                .anchor(egui::Align2::LEFT_TOP, egui::vec2(8.0, 8.0))
                                .show(ctx, |ui| {
                                    ui.label("moho overlay");
                                    ui.horizontal(|ui| {
                                        let load_resp = ui.add(egui::Button::new("Load Scene"));
                                        let exit_resp = ui.add(egui::Button::new("Exit"));

                                        // ALWAYS remember the response rects so our
                                        // fallback hit-testing and diagnostics use the
                                        // exact same geometry that egui painted.
                                        self.load_button_rect = Some(load_resp.rect);
                                        self.exit_button_rect = Some(exit_resp.rect);

                                        if load_resp.clicked() {
                                            let _ = self.sender.send(UiEvent::LoadScene(PathBuf::from("saves/scene.bin")));
                                            // Close overlay internally and notify application
                                            // to manage OS cursor/grab via OverlayToggled.
                                            self.ui_visible = false;
                                            self.cursor_grabbed = Some(true);
                                            let _ = self.sender.send(UiEvent::OverlayToggled(false));
                            ...
                            egui::TopBottomPanel::top("moho_top_panel").show(ctx, |ui| {
                                ui.horizontal(|ui| {
                                    ...
                                    if exit_resp.clicked() {
                                        if cfg!(feature = "ui-egui-debug") {
                                            log::info!("EguiUi: Exit button clicked (noop for debug)");
                                        }
                                        // No-op: do not send Exit while debugging input
                                            ui_emitted_action = true;
                                    }
                ...
                if self.mouse_was_pressed && !self.mouse_pressed {
                    if let Some((px, py)) = self.mouse_press_pos.take() {
                        if let Some((rx, ry)) = self.last_cursor {
                            let in_load_press = self.load_button_rect.map_or(false, |r| r.contains(egui::pos2(px, py)));
                            let in_exit_press = self.exit_button_rect.map_or(false, |r| r.contains(egui::pos2(px, py)));
                            let in_load_release = self.load_button_rect.map_or(false, |r| r.contains(egui::pos2(rx, ry)));
                            let in_exit_release = self.exit_button_rect.map_or(false, |r| r.contains(egui::pos2(rx, ry)));
                        ...
                        let in_load = self.load_button_rect.map_or(false, |r| r.contains(egui::pos2(x, y)));
                        let in_exit = self.exit_button_rect.map_or(false, |r| r.contains(egui::pos2(x, y)));
                        if !ui_emitted_action {
                            if in_load {
                                let _ = self.sender.send(UiEvent::LoadScene(PathBuf::from("saves/scene.bin")));
                                self.ui_visible = false;
