...
fn render_start_menu_ui(ui: &mut egui::Ui) -> Option<MenuAction> {
    ...
    ui.vertical_centered(|ui| {
        ...
        // Continue button
        if ui.add_enabled(save_exists, egui::Button::new("Continue").min_size(egui::vec2(160.0, 32.0))).clicked() {
            action = Some(MenuAction::Continue);
        }
        ui.add_space(8.0);
        
        // New World button  
        if ui.add(egui::Button::new("New World").min_size(egui::vec2(160.0, 32.0))).clicked() {
            action = Some(MenuAction::NewWorld);
        }
        ui.add_space(8.0);
        
        // Settings button (placeholder)
        if ui.add(egui::Button::new("Settings").min_size(egui::vec2(160.0, 32.0))).clicked() {
            action = Some(MenuAction::Settings);
        }
        ui.add_space(8.0);
        
        // Exit button
        if ui.add(egui::Button::new("Exit").min_size(egui::vec2(160.0, 32.0))).clicked() {
...
impl EguiUi {
    pub fn new(window: Option<Arc<Window>>) -> (Self, UiReceiver) {
        ...
        let egui_winit = if let Some(win) = &window {
        ...
        if let Some(action) = button_clicked {
            match action {
                MenuAction::Continue => {
                    ...
                    let _ = self.sender.send(UiEvent::LoadScene("saves/scene.bin".into()));
